...
document.getElementById("saveLwaContent").onclick = () => {
  const item = {
    ...
    comments:Number(document.getElementById("lwaContentComments").value || 0), leads:Number(document.getElementById("lwaContentLeads").value || 0),
    didWell:document.getElementById("lwaContentDidWell").checked, notes:document.getElementById("lwaContentNotes").value.trim()
  };
...
document.getElementById("saveChbContent").onclick = () => {
  const item = {
    ...
    comments:Number(document.getElementById("chbContentComments").value || 0), leads:Number(document.getElementById("chbContentLeads").value || 0),
    didWell:document.getElementById("chbContentDidWell").checked, notes:document.getElementById("chbContentNotes").value.trim()
  };
  if(!item.title) return;
...
function contentCard(item, prefix, pillars){
  return `<div class="item"><div class="inline"><strong>${escapeHtml(item.title)}</strong><span class="badge platform">${item.platform}</span><span class="badge">${item.pillar}</span><span class="badge ${item.status === "Posted" ? "success" : ""}">${item.status}</span>${item.didWell ? '<span class="badge success">Did Well</span>' : '<span class="badge warn">Needs Work</span>'}</div><div class="metrics"><div class="metric"><div class="label">Platform</div><div class="value">${item.platform}</div></div><div class="metric"><div class="label">Touches</div><div class="value">${item.touches}</div></div><div class="metric"><div class="label">Comments</div><div class="value">${item.comments || 0}</div></div><div class="metric"><div class="label">Leads</div><div class="value">${item.leads || 0}</div></div></div><div class="grid two" style="margin-top:10px"><input data-content-title="${prefix}-${item.id}" value="${escapeHtml(item.title)}" /><select data-content-platform="${prefix}-${item.id}">${CONTENT_PLATFORMS.map(p => `<option value="${p}" ${p===item.platform?"selected":""}>${p}</option>`).join("")}</select><select data-content-pillar="${prefix}-${item.id}">${pillars.map(p => `<option value="${p}" ${p===item.pillar?"selected":""}>${p}</option>`).join("")}</select><select data-content-status="${prefix}-${item.id}">${["Planned","Drafted","Posted"].map(s => `<option value="${s}" ${s===item.status?"selected":""}>${s}</option>`).join("")}</select><input type="date" data-content-date="${prefix}-${item.id}" value="${item.date}" /><input type="number" min="0" data-content-touches="${prefix}-${item.id}" value="${item.touches}" /><input type="number" min="0" data-content-comments="${prefix}-${item.id}" value="${item.comments || 0}" /><input type="number" min="0" data-content-leads="${prefix}-${item.id}" value="${item.leads || 0}" /></div><label class="inline-check"><input type="checkbox" data-content-didwell="${prefix}-${item.id}" ${item.didWell ? "checked" : ""} /> Did it do well?</label><textarea data-content-notes="${prefix}-${item.id}" placeholder="Notes">${escapeHtml(item.notes || "")}</textarea><div class="inline"><button onclick="saveContentItem('${prefix}', ${item.id})">Save</button><button class="secondary" onclick="updateContentTouches('${prefix}', ${item.id}, 1)">+1 Touch</button></div></div>`;
}
...
window.saveContentItem = function(prefix, idVal){
  ...
  item.comments = Number(document.querySelector(`[data-content-comments="${prefix}-${idVal}"]`).value || 0);
  item.leads = Number(document.querySelector(`[data-content-leads="${prefix}-${idVal}"]`).value || 0);
  item.didWell = document.querySelector(`[data-content-didwell="${prefix}-${idVal}"]`).checked;
