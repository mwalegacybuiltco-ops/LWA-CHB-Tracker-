...
const STORAGE = {
  lwaLeads: "lb_static_lwa_leads",
  lwaContent: "lb_static_lwa_content",
  chbLeads: "lb_static_chb_leads",
  chbContent: "lb_static_chb_content",
};
...
const FOLLOWUPS = ["24 Hours","48 Hours","72 Hours","30 Day"];
...
let state = {
  ...
  lwaLeads: load(STORAGE.lwaLeads, [
    {id:id(),name:"Sarah M.",platform:"Facebook",stage:"24 Hours",followType:"24 Hours",followDate:today(),score:7,joinedVIP:true,becameAffiliate:false,becameAmbassador:false,ghosted:false,notes:"Watched the overview.",history:[{id:id(),text:"Sent first follow-up.",createdAt:new Date().toISOString()}]},
  ]),
  ...
  chbLeads: load(STORAGE.chbLeads, [
    {id:id(),name:"Megan P.",platform:"Facebook",stage:"48 Hours",followType:"48 Hours",followDate:today(),score:6,joinedVIP:false,becameAffiliate:false,becameAmbassador:false,ghosted:false,notes:"Asked about boutique products.",history:[{id:id(),text:"Sent intro message.",createdAt:new Date().toISOString()}]},
  ]),
...
["lwaLeadFollowType","chbLeadFollowType"].forEach(idName => fillSelect(idName, FOLLOWUPS));
...
document.getElementById("saveLwaLead").onclick = () => {
  const item = {
    ...
    followType:document.getElementById("lwaLeadFollowType").value,
...
document.getElementById("saveChbLead").onclick = () => {
  const item = {
    ...
    followType:document.getElementById("chbLeadFollowType").value,
...
document.getElementById("exportBtn").onclick = () => {
  if(state.section === "lwa"){
    csvDownload("lwa-leads.csv", [
      ["Name","Platform","Stage","Follow Type","Follow Date","Score","VIP","Affiliate","Ambassador","Ghosted","Notes"],
      ...filteredLeads(state.lwaLeads, state.lwaFilter).map(x => [x.name,x.platform,x.stage,x.followType,x.followDate,x.score,x.joinedVIP,x.becameAffiliate,x.becameAmbassador,x.ghosted,x.notes])
    ]);
  }else{
    csvDownload("canine-haven-leads.csv", [
      ["Name","Platform","Stage","Follow Type","Follow Date","Score","VIP","Affiliate","Ambassador","Ghosted","Notes"],
      ...filteredLeads(state.chbLeads, state.chbFilter).map(x => [x.name,x.platform,x.stage,x.followType,x.followDate,x.score,x.joinedVIP,x.becameAffiliate,x.becameAmbassador,x.ghosted,x.notes])
    ]);
  }
};
...
function filteredLeads(arr, filter){
  return arr.filter(item => {
    const q = state.query;
    const matchesSearch = !q || [item.name,item.platform,item.stage,item.notes,item.followType].join(" ").toLowerCase().includes(q);
    ...
    else if(filter !== "All") matchesFilter = item.followType === filter || item.stage === filter;
...
function renderCalendar(elId, arr){
  ...
  return `
    <div class="item">
      ...
      <div class="grid two">
        ...
        <select data-field="${prefix}-followType-${item.id}">${FOLLOWUPS.map(s => `<option ${s===item.followType?"selected":""}>${s}</option>`).join("")}</select>
...
window.saveLeadCard = function(prefix, idVal){
  ...
  item.followType = document.querySelector(`[data-field="${prefix}-followType-${idVal}"]`).value;
...
function render(){
  renderStats();
  renderCalendar("lwaCalendar", filteredLeads(state.lwaLeads, state.lwaFilter));
  renderCalendar("chbCalendar", filteredLeads(state.chbLeads, state.chbFilter));
