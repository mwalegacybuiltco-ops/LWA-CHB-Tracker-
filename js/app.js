...
const STORAGE={lwaLeads:"lbv2_lwa_leads",lwaContent:"lbv2_lwa_content",bizLeads:"lbv2_biz_leads",bizContent:"lbv2_biz_content",todos:"lbv2_todos",brand:"lbv2_brand",dashboard:"lbv2_dashboard"};
...
document.getElementById("installBtn").onclick=async()=>{if(!deferredPrompt)return;await deferredPrompt.prompt();deferredPrompt=null;document.getElementById("installBtn").classList.add("hidden");};
...
const downloadCSV=(filename, rows)=>{const csv=rows.map(r=>r.map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(",")).join("\n"); const blob=new Blob([csv],{type:"text/csv;charset=utf-8;"}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=filename; a.click(); URL.revokeObjectURL(url);};
...
let state={
  ...
  brand:load(STORAGE.brand,{businessName:"Your Business",ownerName:"",tagline:""}),
  dashboard:load(STORAGE.dashboard,{
  ...
  lwaLeads:load(STORAGE.lwaLeads,[{id:id(),name:"Sarah M.",platform:"Facebook",stage:"24 Hours",followDate:today(),score:7,joinedVIP:true,becameAffiliate:false,becameAmbassador:false,ghosted:false,notes:"Watched the overview.",history:[{id:id(),text:"Sent first follow-up.",createdAt:new Date().toISOString()}]},{id:id(),name:"Jen K.",platform:"TikTok",stage:"Ghosted",followDate:today(),score:3,joinedVIP:false,becameAffiliate:false,becameAmbassador:false,ghosted:true,notes:"Opened then disappeared.",history:[]}]),
  lwaContent:load(STORAGE.lwaContent,[{id:id(),title:"What digital marketing actually is",platforms:["TikTok"],pillar:"Authority",status:"Drafted",date:today(),touches:1,comments:4,leads:1,didWell:true,notes:"Simple explanation."}]),
  bizLeads:load(STORAGE.bizLeads,[{id:id(),name:"New prospect",platform:"Facebook",stage:"48 Hours",followDate:today(),score:6,joinedVIP:false,becameAffiliate:false,becameAmbassador:false,ghosted:false,notes:"Interested in the business.",history:[{id:id(),text:"Sent intro message.",createdAt:new Date().toISOString()}]}]),
  bizContent:load(STORAGE.bizContent,[{id:id(),title:"Welcome post",platforms:["Facebook","Instagram"],pillar:"Story",status:"Planned",date:today(),touches:0,comments:0,leads:0,didWell:false,notes:"Intro to the brand."}]),
  todos:load(STORAGE.todos,[{id:id(),title:"Follow up with Sarah",business:"LWA",date:today(),priority:"High",notes:"Check if she watched the overview.",done:false}]),
};

function persist(){
  save(STORAGE.brand,state.brand); save(STORAGE.dashboard,state.dashboard);
  save(STORAGE.lwaLeads,state.lwaLeads); save(STORAGE.lwaContent,state.lwaContent);
  save(STORAGE.bizLeads,state.bizLeads); save(STORAGE.bizContent,state.bizContent);
  save(STORAGE.todos,state.todos);
}
function fillSelect(idName, options){document.getElementById(idName).innerHTML=options.map(o=>`<option value="${o}">${o}</option>`).join("");}
["lwaLeadStage","bizLeadStage"].forEach(id=>fillSelect(id,STAGES));
fillSelect("lwaContentPillar",LWA_PILLARS); fillSelect("bizContentPillar",BIZ_PILLARS);
["lwaLeadFollowDate","bizLeadFollowDate","lwaContentDate","bizContentDate","todoDate"].forEach(id=>document.getElementById(id).value=today());
...
document.getElementById("saveReflections").onclick=()=>{state.dashboard.reflections={win:reflectionWin.value,leak:reflectionLeak.value,support:reflectionSupport.value,move:reflectionMove.value};persist();render();};
...
function renderDailyTracker(){
  dashboardWeekLabel.value = state.dashboard.weekLabel || "";
  dashboardWeekHeading.textContent = state.dashboard.weekLabel || "Weekly Dashboard";
  dailyTrackerBody.innerHTML = state.dashboard.daily.map((row, idx)=>`
    <tr>
  ...
  dailyTrackerTotalRow.innerHTML = `
...
function getAllCalendarItems(){
  const bn=state.brand.businessName||"Your Business";
  return [...state.lwaLeads.map(x=>({...x,business:"LWA"})),...state.bizLeads.map(x=>({...x,business:bn}))];
}
...
function renderSelectedDay(){
  ...
  selectedDayItems.innerHTML=items.length?items.map(item=>`<div class="item"><div class="inline"><strong>${escapeHtml(item.name)}</strong><span class="badge">${item.business}</span><span class="badge">${item.stage}</span></div><div class="small muted">${escapeHtml(item.platform)} • ${item.followDate}</div><div class="small">${escapeHtml(item.notes||"")}</div></div>`).join(""):`<div class="item">No follow-ups on ${state.calendarDate}.</div>`;
}
...
function render(){
  ...
  renderStats();
  renderDailyTracker();
  renderMetricList("affiliateHealthList", state.dashboard.affiliateHealth, "affiliateHealth");
  renderMetricList("ambassadorPipelineList", state.dashboard.ambassadorPipeline, "ambassadorPipeline");
  reflectionWin.value = state.dashboard.reflections.win || "";
  reflectionLeak.value = state.dashboard.reflections.leak || "";
  reflectionSupport.value = state.dashboard.reflections.support || "";
  reflectionMove.value = state.dashboard.reflections.move || "";

  renderFlow("lwaFlow",state.lwaLeads,"lwa");
  renderFlow("bizFlow",state.bizLeads,"biz");
  renderQuickCalendar("lwaCalendar",filteredLeads(state.lwaLeads,state.lwaFilter));
  renderQuickCalendar("bizCalendar",filteredLeads(state.bizLeads,state.bizFilter));
  renderContent("lwaContentList",state.lwaContent,"lwa",LWA_PILLARS);
  renderContent("bizContentList",state.bizContent,"biz",BIZ_PILLARS);
  renderLeads("lwaLeadList",state.lwaLeads,state.lwaFilter,"lwa");
  renderLeads("bizLeadList",state.bizLeads,state.bizFilter,"biz");
  renderMonthCalendar();
  renderSelectedDay();
  renderTodos();
}
